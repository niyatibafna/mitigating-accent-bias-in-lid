...
from et_dists_and_transcribe_dataset import main as load_or_compute_dists_and_transcribe_dataset
...
def parse_args():
    ...
    parser.add_argument("--lid_model_dir", type=str, default=None, help="Directory containing the trained LID model")
    ...
    parser.add_argument("--output_dir", type=str, default=None, help="Output directory for LID model")
...
def combine_and_predict(lid_model, test_dataset, collate_fn):
    '''Combine the predictions from the model with the predictions from the ET model and evaluate'''
    softmax = torch.nn.Softmax(dim=0)
    test_loader = DataLoader(test_dataset, batch_size=lid_model.batch_size, shuffle=False, collate_fn=collate_fn)
    lid_model.model.eval()
    all_labels = []
    all_preds = []
    all_accents = []
    all_audio_files = []
    for batch in test_loader:
        ...
        with torch.no_grad():
            ...
            # Normalize the ET output distributions to sum up to 1
            et_output_dists = [output_dist / output_dist.sum() for output_dist in et_output_dists]
            # et_output_dists = torch.norm(et_output_dists, p=1, dim=2)

            combined_dists = [0.5 * output.cpu() + 0.5 * et_output_dist for output, et_output_dist in zip(outputs, et_output_dists)]
            combined_dists = torch.stack(combined_dists)
            preds = torch.argmax(combined_dists, dim=1)
...
def load_dists_and_phoneseq_dataset(transcriber_model, encoder_model, dataset_name, \
    ...
    for lang in langs:
        ...
        lang_dataset = load_or_compute_dists_and_transcribe_dataset(transcriber_model, encoder_model, dataset_name, per_lang=per_lang, lang=lang, \
...
def main():
    ...
    assert only_eval, "This script is only for inference. Phoneseq model is done separately, acoustic model reps are assumed to be already computed as part of dataset."
    assert load_trained_from_dir, "If only evaluating, you must load the phoneseq model from a directory"
    ...
    if lid_model_type == "attentions-linear":
        ...
        lid_model = PhoneseqsLinearClassifiereonAttentionLayers(vocab_size=vocab_size, num_classes=num_classes, \
                                hidden_size=hidden_size, num_attention_layers=num_attention_layers, \
                                    attention_dim=128, \
                                    load_from_dir = load_trained_from_dir, output_dir = lid_model_dir, \
    ...
    if eval_dataset_name:
        ...
        # Replace this function to load the eval dataset
        ## Use similar loading logic to the reps_and_phoneseq logs
        eval_dataset = load_dists_and_phoneseq_dataset(transcriber_model, encoder_model, eval_dataset_name, langs, per_lang=per_lang, batch_size=batch_size, output_dir=save_eval_dataset_dir, log_file=None)
