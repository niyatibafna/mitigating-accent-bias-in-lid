...
def prepare_dataset(batch):
    ...
    # Split audio by phoneme timestamps
    new_array_values = []
    labels = []
    for array, timestamp_file, in zip(array_values, batch["timestamp_file"]):
        timestamps = get_labels_from_timestamp_file(timestamp_file)
        for timestamp in timestamps:
            start, end, label = int(timestamp[0]), int(timestamp[1]), timestamp[2]
            new_array_values.append(array[start:end])
            labels.append(label)
...
for data in dataset.iter(batch_size = batch_size):
    ...
    max_len = max([len(signal) for signal in data["signal"]])
    signals = torch.stack([torch.nn.functional.pad(torch.tensor(signal), (0, max_len - len(signal))) for signal in data["signal"]])
...
np.save(os.path.join(output_dir, f"phone-reps_precise_et_layer-final.npy"), all_reps)
np.save(os.path.join(output_dir, f"phone-labels_precise_et_layer-final.npy"), np.array(all_labels))
...
plt.show()
plt.savefig(f"{output_dir}/phone-clusters_precise_layer-final.png", bbox_inches='tight')
