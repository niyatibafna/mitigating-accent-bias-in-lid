...
class PhoneseqsLinearClassifiereonAttentionLayers(PostEncoder):
    ...
    def train(self, train_dataset, dev_dataset, evaluate_steps = None):
        ...
        wandb.init(project=f"train_lid_on_phoneseqs_attentions-{self.model.num_attention_layers}", config={
...
def parse_args():
    ...
    parser.add_argument("--dataset_dir", type=str, required=True, help="Directory containing audio files")
...
def map_tokenize_phoneme_labels(batch, phoneme2idx, lang2idx):
    ...
    '''
    phoneme_sequences = [item["phone_sequence"] for item in batch]
    ...
    batch["labels"] = [lang2idx[lang] for lang in batch["lang"]]
...
def main():
    ...
    if not only_eval:
        ...
        lid_dataset = lid_dataset.map(map_tokenize_phoneme_labels, fn_kwargs={"phoneme2idx": phoneme2idx, "lang2idx": lang2idx}, batched=True, \
            batched=True, \
            batch_size = 1000)
    ...
    if eval_dataset_dir:
        ...
        eval_dataset = eval_dataset.map(map_tokenize_phoneme_labels, fn_kwargs={"phoneme2idx": phoneme2idx, "lang2idx": lang2idx}, batched=True, \
            batched=True, \
            batch_size = 1000)
