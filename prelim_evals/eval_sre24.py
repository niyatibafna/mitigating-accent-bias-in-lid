...
def load_sre24(num_samples = None):
    accents = {
        "ara-apc",
        "ara-acm",
        "ara-ary",
    ...
    all_data = []
    with open("/exp/jvillalba/corpora/LDC2024E34/sre24_devset_docs/docs/sre24_audio_dev_segment_key.tsv") as f:
        for line in f:
            path, accent, split = line.strip().split("\t")[0], line.strip().split("\t")[5], line.strip().split("\t")[6]
            if accent.strip() not in accents:
                continue
            filepath = f"/exp/jvillalba/corpora/LDC2024E12/sre24_devset/data/{split.strip()}/{path.strip()}"
...
preds_df = df(preds)
print(preds_df)
preds_df.to_csv("sre24_preds.csv")
