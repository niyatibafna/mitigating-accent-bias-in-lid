...
class RepsDUseqsLinearClassifiereonAttentionLayersModel(torch.nn.Module):
    '''
    Linear classifier on pooled representations
    '''
    def __init__(self, num_classes, hidden_size, num_attention_layers, attention_dim, reps_dim):
        super(RepsDUseqsLinearClassifiereonAttentionLayersModel, self).__init__()
...
class RepsDUseqsLinearClassifiereonAttentionLayers(PostEncoder):
    ...
    def __init__(self, num_classes, hidden_size, num_attention_layers, attention_dim, load_from_dir = False,  output_dir = None, batch_size = None, lr = None, num_epochs = None,\
        ...
        if load_from_dir:
            ...
            model = RepsDUseqsLinearClassifiereonAttentionLayersModel(num_classes=num_classes, hidden_size=hidden_size, \
    ...
    def train(self, train_dataset, dev_dataset, evaluate_steps = None):
        if "wav2vec2-base-layer8" in self.output_dir:
            ...
            kmeans_units = None
        wandb.init(project=f"train_lid_on_reps_duseqs_attentions-{self.model.num_attention_layers}", config={
...
def map_get_codevectors_reps(batch):
    ...
    global centroids
    centroid_sequences = batch["sequences"]
    # centroids = batch["centroids"]
    codevector_reps = centroids[centroid_sequences]
    batch["kmeans_reps_embedded"] = codevector_reps
    return batch
...
def collate_fn(batch):
    ...
    ssl_inputs = torch.stack([torch.tensor(item["kmeans_reps_embedded"]) for item in batch])
...
def main():
    ...
    if not only_eval:
        ...
        lid_dataset.set_transform(map_get_codevectors_reps)
    ...
    if lid_model_type == "attentions-linear":
        ...
        lid_model = RepsDUseqsLinearClassifiereonAttentionLayers(num_classes=num_classes, hidden_size=hidden_size, num_attention_layers=num_attention_layers, \
